...
//   // TODO: 여기에 코드를 작성합니다.
//   // initVal 초기값(누적되는 값)
//   _.each(arr, function(acc, cur){
//     if(initVal === undefined && cur === 0){ //initVal이 없고, index가 0번째면...
//       initVal = acc; // 처음에 초기값 넣기!
//     } else {
...
//-------------------------------------------------------------
_.reduce = function (arr, iteratee, initVal) {
  // 초기값(initVal)이 있으면 누적값(acc)은 초기값
  let acc = initVal;
  _.each(arr, (elem, idx, arr) => {
    // 만약에 initVal이 없으면 배열의 0번째 요소를 acc에 할당
    if(initVal === undefined && idx === 0){
      acc = elem;
    } else {
      // 누적값(acc)에 함수를 실행시킨 결과값 할당
      // (누적값=acc, 현재값=elem, 인덱스=idx ,초기배열=arr)
      acc = iteratee(acc, elem, idx, arr);
    }
  });
  return acc;
};