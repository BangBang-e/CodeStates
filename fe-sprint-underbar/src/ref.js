...
_.slice = function (arr, start, end) {
    ...
    if (start < 0) _start = Math.max(0, arr.length + start); // 5 + -2 = 3
...
// ! 아웃풋 예시: [1, 2, 3], 5 -> [1, 2, 3, undefined, undefined]
...
// ! input: [1, 2, 3], 생략 -> output: [1, 2, 3]
// ! input: [1, 2, 3], 2 -> output: [3]
// ! input: [1, 2, 3, 4, 5, 6, 7], 4 -> output: [5, 6, 7]
...
//!  input [1, 2, 3, 4, 5, 6, 7], 3 -> output: [5, 6, 7]
...
// ! interviewer / interviewee 면접관 / 면접자 (면접 당하는 사람)
// ! iterator / iteratee 반복을 해주는 함수 / 반복을 당하는 함수
_.filter = function (arr, test) {
    // TODO: 여기에 코드를 작성합니다.
    let result = []

    _.each(arr, (ele) => {
        if (test(ele)) {  // 얘가 truthy한 값이면 
            result.push(ele)
        }
    })
...
_.reject = function (arr, test) {
    ...
    _.each(arr, (ele) => {
        if (test(ele) === false) {  // 얘가 falsy한 값이면 
            result.push(ele)
        }
    })
...
// ? 궁금한게 있는데 push는 Array 내장 메소드가 아닌가요?
...
// ! input [{ name: 'moe', age: 30 }, { name: 'curly', age: 50 }], 'name'
// ! output ['moe', 'curly']
...
_.pluck = function (arr, keyOrIdx) {
    ...
    // _.each(arr, function (item) { 
      // arr: [{ name: 'moe', age: 30 }, { name: 'curly', age: 50 }]
      // item: { name: 'moe', age: 30 }
      // item['name']: 'moe'
...
//? Array 내장 메소드를 이용하지 않고 언더바 메소드를 구현하라
//? 라는 뜻을 push같은 메소드를 사용하지 않고 구현하라 라고 생각했습니다..
...
_.reduce = function (arr, iteratee, initVal) {
    // TODO: 여기에 코드를 작성합니다.
    // 초기값(initVal)이 있으면 누적값(acc)는 초기값
    let acc = initVal
    _.each(arr, (element, index, arr) => {
        // 만약에 initVal가 없으면 배열의 0번째 요소를 acc에 할당
        if (initVal === undefined && index === 0) {
            acc = element
        } else {
            // 누적값(acc)에 함수를 실행시킨 결과값 할당
            // (누적값=acc, 현재값=element, 인덱스=idx, 초기배열=arr)
            acc = iteratee(acc, element, index, arr)
        }
    })
    return acc
};
// ! acc에 iteratee를 할당하는데 왜 자동으로 값이 누적되나요 ..?